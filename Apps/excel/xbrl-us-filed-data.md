...
description: Todas as informações de segurança e conformidade disponíveis para Dados Arquivados XBRL, suas políticas de tratamento de dados, suas Microsoft Cloud App Security de catálogo de aplicativos e informações de segurança/conformidade no Registro STAR do CSA.
...
| Funcionalidade principal do aplicativo | A XBRL US é a criação de um consorciado sem fins lucrativos que dá suporte a padrões de relatórios comerciais gratuitos, abertos e não apropriados nos EUA XBRL US tem conduzido o desenvolvimento e a implementação de padrões de dados para agências governamentais - a Comissão de Valores Mobiliários e Exchange (SEC), a Comissão Federal De Regulação de Energia (FERC) e o Departamento de Energia. O XBRL US monitora dados e ajuda a criar padrões de qualidade de dados usados pela SEC para ajudar as empresas públicas a criar demonstrações financeiras XBRL de boa qualidade.  |
